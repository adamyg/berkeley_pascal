 ...
 */
out_t	Outchar = termchar;
out_t	Put_char = normchar;
out_t	Pline = normline;
...
flusho()
{
	...
	if (obp != obuf) {
#ifndef vms
		write(ex_stdout /*2*/, obuf, obp - obuf);
#else
		vms_write(ex_stdout /*2*/, obuf, obp - obuf);
#endif
		obp = obuf;
	}
}
...
void
lvprintf(const char *fmt, va_list ap)
{
	out_t P;
