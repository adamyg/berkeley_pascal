...
long	cntch = 0;		/* Count of characters on unit io */
#ifndef VMUNIX
short	cntln = 0;		/* Count of lines " */
#else
int	cntln = 0;
#endif
long	cntnull = 0;		/* Count of nulls " */
long	cntodd = 0;		/* Count of non-ascii characters " */
...
bool dofname;	/* if 1 call filename, else use savedfile */
{
	...
	if (!*iop && !nonexist){
		*lp = 0 ;
		if ((*iop = open(file, O_WRONLY /*1*/)) < 0) *iop = 0 ;
	}
#endif  /*FLOCKFILE*/
	switch (c) {
...
#else
#if defined(_MSC_VER)
		io = creat(file, _S_IREAD|_S_IWRITE);
#else
		io = creat(file, 0666);
#endif
		...
		if (--ninbuf < 0) {
			...
			if (kflag) {
				...
				while(fp < &genbuf[ninbuf]) {
					if (*fp++ & 0200) {
						crblock(perm, genbuf, ninbuf+1, cntch);
						break;
					}
				}
			}
...
{
	jmp_buf osetexit;
	register int saveinp, ointty, oerrno, newin;
	char *saveglobp;
	short savepeekc;
	...
	if (slevel <= 0)
		ttyindes = saveinp;
	close(0);
	if ((newin = open(fil, O_RDONLY /*0*/)) < 0) {
		oerrno = errno;
		setrupt();
		dup(saveinp);
		close(saveinp);
...
#ifdef FDEBUG
	if (trace)
		tfixnl(), fprintf(trace, "<SOURCE(%s):%d>\n", fil, newin);
#endif
        saveinp = ex_stdin; ex_stdin = newin;
	if (slevel <= 0)
		ttyindes = saveinp;
	slevel++;
	ointty = intty;
	intty = isatty(ex_stdin);
	oprompt = value(PROMPT);
	value(PROMPT) &= intty;
	getexit(osetexit);
	setrupt();
	if (setexit() == 0)                     /* setjmp(resetlab) */
		commands(1, 1);
	else if (slevel > 1) {
                ex_stdin = saveinp;
		slevel--;
		resexit(osetexit);
		reset();                        /* longjmp(resetlab) */
	}
#ifdef FDEBUG
	if (trace)
		tfixnl(), fprintf(trace, "<CLOSE(%s):%d>\n", fil, newin);
#endif
	intty = ointty;
	value(PROMPT) = oprompt;
	globp = saveglobp;
