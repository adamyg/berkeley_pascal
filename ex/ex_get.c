...
top:
	...
	flush();
	if (intty) {
		c = read(ex_stdin, inputline, sizeof inputline - 4);
		if (c < 0) {
#ifdef FDEBUG
			if (trace)
				fprintf(trace, "<EOF:%d>\n", ex_stdin);
	...
	c = read(ex_stdin, inputline, sizeof inputline - 1);
	if(c <= 0) {
#ifdef FDEBUG
		if (trace)
			fprintf(trace, "<EOF:%d>\n", ex_stdin);
	...
	if (intty && !inglobal) {
		...
		if (value(AUTOINDENT) ^ aiflag) {
			...
			while ((c = getcd()) == CTRL('d')) {
				if (lastin == 0 && isatty(ex_stdin /*0*/) == -1) {
					holdcm = 0;
					return (EOF);
				}
...
const char	*cntrlhm =	"^H discarded\n";
