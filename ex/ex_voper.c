	...
	register int c, cnt;
{
	...
	moveop = vmoveop, deleteop = vdelete;
	wcursor = cursor;
	wdot = NOLINE;
	notecnt = 0;
	dir = 1;
...
nocount:
	...
	case 'w':
		wdkind = c & ' ';
		forbid(xlfind(2, cnt, opf, (line *) 0) < 0);
		vmoving = 0;
		break;
	...
	case 'e':
		wdkind = 1;
ein:
		forbid(xlfind(3, cnt - 1, opf, (line *) 0) < 0);
		vmoving = 0;
		break;
	 ...
	 */
	case ')':
		forbid(xlfind(0, cnt, opf, (line *) 0) < 0);
		markDOT();
		break;
	 ...
	 */
	case '}':
		forbid(xlfind(1, cnt, opf, (line *) 0) < 0);
		markDOT();
		break;
...
fixup:
	...
	case 'l':
	case ' ':
		forbid (margin() || opf == vmoveop && edge());
		while (cnt > 0 && !margin())
			wcursor += dir, cnt--;
		if (margin() && opf == vmoveop || wcursor < linebuf)
			wcursor -= dir;
		vmoving = 0;
		break;
...
errlab:
	...
	case '`':
		...
		if (opf == vmoveop && (wdot != dot || (d == '`' && wcursor != cursor)))
			markDOT();
		if (wcursor) {
			vsave();
			ex_getline(*wdot);
	...
	int cnt;
{
	...
	if (dir == 1) {
		...
		while (wordof(which, wcursor)) {
			if (cnt == 1 && op != vmoveop && wcursor[1] == 0) {
				wcursor++;
				break;
			}
			if (!lnext())
