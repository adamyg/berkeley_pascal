program nmaze(input, output);

const 	maxwidth 	= 122;
	maxheight 	= 52;			{lpt page size}
    	n		= 4;
	nfact		= 24;
...
procedure perle(var s:row);
	{	s is a row consisting of the nth permutation,
...
begin
	for i:=1 to n do
		table[1][i]:=i;			{initialize first row}
	for i:=2 to nfact do		
	begin
...
begin
	...
	pip:= seed(pip);
	for i:=-1 to 2*width+1 do
		for j:=-1 to 2*height+1 do
			maze[i,j]:=wall;
	for i:=-2 to 2*width+2 do
	begin
		maze[i,-2]:=border;
		maze[i,2*height+2]:=border
	end;
	for j:= -2 to 2*height+2 do
	begin
		maze[-2,j]:=border;
		maze[2*width+2,j]:=border
	end;
end;
...
begin	{growtree}
	...
	if (x=-2) and (touchl=false) then
	begin
		touchl:=true; 			{touched left border}
		fillin
	end;
	if (x=2*width+2) and (touchr=false) then
	begin
		touchr:=true;
		fillin
	end
...
var 	i,j:integer;
begin
	for j:= 2*height+1 downto -1 do begin
		write(' ');
		for i:=-1 to 2*width+1 do
			case maze[i,j] of
			path:	write(' ');
