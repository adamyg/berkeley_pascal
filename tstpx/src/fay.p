{ AN OPEN CHALLENGE TO ALL PASCAL COMPILERS:
	Compile this program correctly and produce the correct output.
...
type	A = array[node] of integer; 	{ range 0..Infinity (below) }
...
var	I,T:	integer;
	...
	begin {Search}
		...
		begin         		{ stack the node }
			...
			if Debug then
				writeln('Doing successors of ',V);
			R(V, DoUsuccessor);

			if Debug then
				writeln('Now checking if ',V,' is an SCC root');
			if V = Stack[N^[V]] then
			begin		{ V is root of an SCC }
				T := N^[V];
	
				if Debug then
					writeln(V,' is an SCC root; SP=',SP,' T=',T);
				for I := SP downto T do 
					N^[Stack[I]] := Infinity;
	      			if SP <> T then
				begin
					if Debug then
						writeln('Yield SCC should pass ',V,' out to TakeUSCC');
					YieldUSCC;
				end;
				SP := T-1;
			end;
...
begin
	...
	for V in EachUnodeUtoUsearch() do
		Search(V);
	dispose(N);
end;
