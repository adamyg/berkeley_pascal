...
void	process(void);
int	match(const char *ocp);
void	copystr(void);
int	octdigit(char c);
void	inithash(void);
int	hashit(const char *str, char really, unsigned fakept);
int	fgetNUL(char *obuf, register int rmdr, FILE *file);

FILE	*mesgread = NULL, *mesgwrite = NULL;
char	*progname = NULL;
char	usagestr[] =	"usage: %s [ - ] mesgfile prefix file ...\n";
char	name[100] = {0}, *np = NULL;
